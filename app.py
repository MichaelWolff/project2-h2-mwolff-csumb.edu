...
def on_connect(data):
    ...
    serverMessage = parseWithChatty(">>"+text)
...
def on_quit(data):
    ...
    serverMessage = parseWithChatty("<<"+data['username'])
    if serverMessage is not None:
        messages.append(json.dumps(serverMessage))
    #Emit the new list of messages to all the clients
    socketio.emit('all users', {
...
def messageRecieved(data):
    ...
    serverMessage = parseWithChatty(data['message']['text'])
...
if __name__ == '__main__':
    socketio.run(app,host=os.getenv('IP','0.0.0.0'),port=int(os.getenv('PORT',5432)),debug=True)
